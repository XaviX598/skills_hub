...
import { ArrowRight, BookOpen, ChevronDown, Download, LogIn, LogOut, Plus, Search, Sparkles, User } from 'lucide-react';
...
const navItems = [
  ...
  { href: '/app', label: 'Desktop App', icon: Download },
];
      ...
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 md:px-6">
        <div className="flex items-center gap-8">
          <Link href="/" className="group flex items-center gap-3" aria-label="Universal Skills Hub home">
            <span className="grid h-9 w-9 place-items-center rounded-2xl border border-cyan-300/25 bg-cyan-300/10 text-[var(--accent-cyan)] shadow-[0_0_30px_rgba(0,212,255,0.2)] transition-all duration-300 group-hover:scale-105 group-hover:shadow-[0_0_40px_rgba(0,212,255,0.4)]">
              <Sparkles className="h-4 w-4 transition-transform duration-300 group-hover:rotate-12 group-hover:scale-110" />
            </span>
            <span className="font-mono text-sm font-black uppercase tracking-[0.22em] text-white transition-colors duration-300 group-hover:text-[var(--accent-cyan)]">
            ...
            <div className="group/menu relative">
              ...
              <div className="invisible absolute left-0 top-full z-50 w-[25rem] translate-y-3 pt-3 opacity-0 transition-all duration-200 group-hover/menu:visible group-hover/menu:translate-y-0 group-hover/menu:opacity-100 group-focus-within/menu:visible group-focus-within/menu:translate-y-0 group-focus-within/menu:opacity-100">
                <div className="overflow-hidden rounded-3xl border border-white/10 bg-[var(--bg-secondary)]/95 p-2 shadow-[0_24px_80px_rgba(0,0,0,0.45)] backdrop-blur-xl">
                  ...
                  <Link href="/agents" className="btn-secondary flex items-center justify-between rounded-2xl px-3 py-2 text-xs font-semibold">
        ...
        <div className="flex items-center gap-3">
          <Link href="/submit" className="group hidden sm:inline-flex sm:items-center sm:gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-sm font-semibold text-[var(--text-secondary)] transition-all duration-300 hover:border-cyan-300/30 hover:bg-cyan-300/12 hover:text-[var(--accent-cyan)]">
          ...
          {session?.user ? (
            <>
<Link href="/profile" className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 py-1 pl-1 pr-3 text-sm font-medium text-[var(--text-secondary)] transition-all duration-200 hover:border-cyan-300/30 hover:text-[var(--accent-cyan)] hover:shadow-[0_0_15px_rgba(0,212,255,0.15)]">
                {session.user.image ? (
                  <Image src={session.user.image} alt={session.user.name || 'Profile'} width={32} height={32} unoptimized className="rounded-full transition-transform duration-200 hover:scale-105" />
                ) : (
            ...
            <Link href="/login" className="group flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-sm font-semibold text-[var(--text-secondary)] transition-all duration-200 hover:border-cyan-300/30 hover:text-[var(--accent-cyan)] hover:shadow-[0_0_15px_rgba(0,212,255,0.15)]">
