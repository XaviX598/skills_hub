...
import { Check, ChevronRight, Crown, Download, ExternalLink, Lock, Monitor, ShieldCheck, Sparkles, Terminal, Zap } from 'lucide-react';
import { PremiumButton } from '../download/premium-button';
...
export const metadata: Metadata = {
  title: 'Desktop App for AI Agent Skills - Skills Hub',
  description: 'Discover the Skills Hub desktop app for Windows. Browse the same synced catalog as the web, install skills in 1 click, and manage premium access with Stripe.',
  keywords: ['desktop app for AI skills', 'Windows app for Claude Code skills', 'Codex skills desktop app', 'AI agent skills installer', 'Skills Hub desktop'],
  openGraph: {
...
if (!isLoggedIn) {
    return (
      <div className="glass-panel rounded-[2rem] p-6 md:p-7">
        ...
        <h2 className="text-2xl font-black tracking-tight">Sign in first</h2>
        <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">
          You can see what the app does, but to download it or buy premium you need to sign in with your account.
        </p>
...
if (!isPremium) {
    return (
      <div className="glass-panel rounded-[2rem] p-6 md:p-7">
        ...
        <h2 className="text-2xl font-black tracking-tight">Buy premium to download the app</h2>
        <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">
          The logic is separated: if you're not premium, you see the offer; if you are, the download appears directly.
        </p>
        ...
        {showSuccess && (
          <div className="mt-5 rounded-2xl border border-emerald-300/30 bg-emerald-300/10 p-4 text-sm text-emerald-100">
            ...
            <p className="mt-2 text-emerald-50/90">Your premium is now active. Refresh the page if you don't see the download yet.</p>
          </div>
        )}
  ...
  return (
    <div className="glass-panel rounded-[2rem] p-6 md:p-7">
      <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--accent-emerald)]">
        <Crown className="h-3.5 w-3.5" />
        Premium active
      </div>
      <h2 className="text-2xl font-black tracking-tight">Download the app now</h2>
      <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">
        You have premium access, so the download is enabled right away. No hassle.
      </p>
<div className="mt-6 flex flex-col gap-3">
        <a href={DESKTOP_RELEASE_URL} className="btn-secondary inline-flex items-center justify-center gap-2 text-base">
          <Download className="h-5 w-5" />
          Download for Windows
        </a>
        <BillingPortalButton className="btn-secondary inline-flex items-center justify-center text-base" />
      </div>
    </div>
  );
}
          ...
          <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
            <div>
              ...
              <h1 className="max-w-4xl text-5xl font-black tracking-tight md:text-7xl">
                Install your premium skills from a{' '}
